    ...
    if (input) {
      input.addEventListener("input", () => {
        const currentDraft = JSON.parse(
          localStorage.getItem("characterDraft") || "{}"
        );
...
headerArr.forEach((element) => {
  element.addEventListener("input", () => {
    localStorage.setItem(
      "header",
      JSON.stringify([character.value, player.value, origin.value, clas.value])
    );
  });
});
...
function loadMainAt() {
  const main_attributes = JSON.parse(
    localStorage.getItem("main-attributes") || "[]"
  );
...
healthAttArr.forEach((element) => {
  element.addEventListener("input", () => {
    localStorage.setItem(
      "health-attributes",
      JSON.stringify([pv.value, san.value, pe.value])
    );
  });
});

function loadHealthAt() {
  const health_attributes = JSON.parse(
    localStorage.getItem("health-attributes") || "[]"
  );
...
defenseAttArr.forEach((element) => {
  element.addEventListener("input", () => {
    localStorage.setItem(
      "defense-attributes",
      JSON.stringify([pas.value, blo.value, esq.value])
    );
  });
});

function loadDefenseAt() {
  const defense_attributes = JSON.parse(
    localStorage.getItem("defense-attributes") || "[]"
  );
...
function montarPayloadPersonagem() {
  ...
  return {
    ...
    character_origin: document.getElementById("character_origin").value,
    character_class: document.getElementById("character_class").value,

    // --- Atributos Principais (Inteiros) ---
    ...
    presence: parseInt(document.getElementById("presence").value) || 0,
    vigor: parseInt(document.getElementById("vigor").value) || 0,

    // --- Saúde e Sanidade (Textos/Strings) ---
    ...
    blo: parseInt(document.getElementById("blo").value) || 0,
    esq: parseInt(document.getElementById("esq").value) || 0,

    // --- Perícias (Inteiros) ---
    atl: parseInt(document.getElementById("atl").value) || 0,
    atu: parseInt(document.getElementById("atu").value) || 0,
    cie: parseInt(document.getElementById("cie").value) || 0,
    dip: parseInt(document.getElementById("dip").value) || 0,
    eng: parseInt(document.getElementById("eng").value) || 0,

    // ATENÇÃO AQUI: O ID no HTML é 'for', mas no banco de dados é 'fort'
    fort: parseInt(document.getElementById("for").value) || 0,

    fur: parseInt(document.getElementById("fur").value) || 0,
    ilu: parseInt(document.getElementById("ilu").value) || 0,
    inti: parseInt(document.getElementById("inti").value) || 0,
    intu: parseInt(document.getElementById("intu").value) || 0,
    inv: parseInt(document.getElementById("inv").value) || 0,
    lut: parseInt(document.getElementById("lut").value) || 0,
    med: parseInt(document.getElementById("med").value) || 0,
    ocu: parseInt(document.getElementById("ocu").value) || 0,
    per: parseInt(document.getElementById("per").value) || 0,
    pil: parseInt(document.getElementById("pil").value) || 0,
    pon: parseInt(document.getElementById("pon").value) || 0,
    pro: parseInt(document.getElementById("pro").value) || 0,
    ref: parseInt(document.getElementById("ref").value) || 0,
    rel: parseInt(document.getElementById("rel").value) || 0,
    tat: parseInt(document.getElementById("tat").value) || 0,
    tec: parseInt(document.getElementById("tec").value) || 0,
    von: parseInt(document.getElementById("von").value) || 0,

    // --- Listas de Relacionamento ---
    abilities_list: local_abilities.map((ab) => ({
      ability_name: ab.name,
      description: ab.input1,
      damage: ab.input2,
      effect: ab.input3,
    })),
